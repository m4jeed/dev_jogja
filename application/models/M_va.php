	...
	function transfer($trx_type,$trx_desc,$amount,$vacc_from,$vacc_to,$pin='',$referral=''){
		...
		$this->db->trans_start();
			...
			if($trx_type=='transfer'||$trx_type=='poin'||$trx_type=='ppob'||$trx_type=='cashout'){
				...
				//var_dump($trx_type);
				$data_from=array('trx_type'=>$trx_type
								,'trx_desc'=>$trx_desc
								,'amount'=>$amount
								,'dk'=>'d'
								,'balance'=>$balance_from
								,'vacc_number'=>$vacc_from
								,'vacc_from'=>$vacc_from
								,'vacc_to'=>$vacc_to
								,'cor_remark'=>$trx_id_from
								,'referral'=>$referral
								,'trx_id2'=>'9'.generateCode()
								);
				$this->db->insert('ma_trx',$data_from);
				$trx_id_from=$this->db->insert_id();
				//var_dump($data_from);die();
			}
			
			$trx_id_to='0';
			//if($trx_type=='transfer'||$trx_type=='poin'||$trx_type=='ppob'||$trx_type=='topup'){
			if($trx_type=='transfer'||$trx_type=='topup'||$trx_type=='poin'){
				//insert ma_trx vacc_to --credit
				$data_to=array('trx_type'=>$trx_type
								,'trx_desc'=>$trx_desc
								,'amount'=>$amount
								,'dk'=>'k'
								,'balance'=>$balance_to
			...
			if($trx_type=='ppob'){
				...
				if($get_referal_code['referal_code']!=''){
					...
					if($vacc_number_upline['vacc_number']!=''){
						...
						//send notif ke referal agent
						$this->load->helper('thirdparty');
						$gcmid[] =$vacc_number_upline['fcm_id'];
						$message_notif='Poin referal dari '.$data_referal_poin['trx_desc'];
						send_notif($gcmid,$message_notif);
					}
				}
