...
class Adm_user extends MY_Jogja {
	...
	public function __construct() 
	{
		...
		if(in_array($lvl,array('ADMIN'))){
			...
			redirect('jogja/not_authorized');
		}
		
		$this->load->model('jogja/M_adm_user','users');
	...
	function ajax_list(){
		...
		$total=$this->users->count_all($search);
		...
		$query=$this->users->get_all($start,$length,$search,$this->column_order[$order], $dir);
		...
		foreach ($query as $row) {
			...
			$output['data'][]=array($nomor_urut
									...
									<a href="javascript:void(0)" class="btn btn-primary btn-sm" onclick="showModalCP('.$user_id.')">Change Password</a>'
									);
			$nomor_urut++;
		}
	...
	public function jsonGetOneData(){
		if($this->input->post()){
			...
			if ($this->form_validation->run() == FALSE){
				...
				$result=$this->users->get_one($user_id);
				if($result){
	...
	public function jsonInsertData(){
		if($this->input->post()){
			...
			if ($this->form_validation->run() == FALSE){
				...
				$result=$this->users->insert_data($data);
				if($result){
	...
	public function jsonUpdateData(){
		if($this->input->post()){
			...
			if ($this->form_validation->run() == FALSE){
				...
				$result=$this->users->update_data($user_id,$data);
				if($result){
	...
	public function jsonCP(){
		if($this->input->post()){
			...
			if ($this->form_validation->run() == FALSE){
				...
				$user_id=strip_tags(trim($this->input->post('user_id',true)));
				$user=$this->users->get_one($user_id);
