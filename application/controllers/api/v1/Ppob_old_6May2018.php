...
class Ppob extends MY_Controller {
	...
	function topup_service_post(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			$ma_trx=$this->db->query("SELECT trx_date,product_name FROM ppob_trx WHERE product_code='".$product_code."' AND user_id='".$user_id."' 
						...
						if(isset($ma_proses['is_error'])){
							if($ma_proses['is_error']=='0'){
								...
								if($TopupService['is_error']=='0'){
									...
									$insert_ppob_trx=$this->db->insert('ppob_trx',$data_ppob_trx);
									//-----------------------------------------
		...
		if ($this->form_validation->run() == FALSE){
			...
			if($idbiller_payment){
				if($idbiller_payment['paid_flag']=='N'){
					...
					$trx_type='ppob';
					$trx_desc='PAY '.$idbiller_payment['product_name'].' | '.$idbiller_payment['customer_number1'].' | '.$idbiller_payment['trx_id'];
